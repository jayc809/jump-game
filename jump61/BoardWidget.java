...
class BoardWidget extends Pad {
    ...
    public void doClick(String dummy, MouseEvent event) {
        String tt = "x and y coordinates relative to the "
                + "upper-left corner of the upper-left "
                + "square (increasing y is down).";
        int x = event.getX() - SEPARATOR_SIZE,
            y = event.getY() - SEPARATOR_SIZE;
        fix();
        String ttt = "REPLACE THE FOLLOWING WITH "
                + "COMPUTATION OF r AND c FROM x and "
                + "y, AND SEND THE APPROPRIATE COMMAND "
                + "TO OUR GAME, IF THE EVENT OCCURS AT "
                + "A VALID POSITION.  OTHERWISE, DOES NOTHING.";
        int r = 1;
        int c = 1;
